...
error_log("Inquiry Form - Raw Input: " . $raw_input);

$input = json_decode($raw_input, true);

if (json_last_error() !== JSON_ERROR_NONE) {
    error_log("Inquiry Form - JSON Error: " . json_last_error_msg());
...
if (!empty($fehler)) {
    error_log("Inquiry Form - Validation Errors: " . implode(', ', $fehler));
    http_response_code(400);
...
error_log("Inquiry Form - Attempting to send email to: " . $empfaenger_email);
error_log("Inquiry Form - From: " . $absender_email);
error_log("Inquiry Form - Subject: " . $betreff);

$mail_result = mail($empfaenger_email, $betreff, $email_inhalt, $headers);

if ($mail_result) {
    error_log("Inquiry Form - Email sent successfully");
    echo json_encode([
        'success' => true, 
    ...
    error_log("Inquiry Form - Email failed. Last error: " . print_r($last_error, true));
